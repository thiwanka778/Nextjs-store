...
const EditStore = () => {
  ...
  const [storeDeleteError,setStoreDeleteError]=React.useState<null|any|string>("");
  ...
  const updateClick = (data: any) => {
    ...
    // console.log(data);
    setStoreInfo({
      storeName: data?.name ? data?.name?.trim() : "",
  ...
  React.useEffect(() => {
    const makeFile = async () => {
      if (fileList?.length >= 1) {
        ...
        const imageURL: string | null | any =
  ...
  React.useEffect(() => {
    if (updateStoreLoading === false) {
      if (updateStoreStatus === true) {
        ...
        } else if (!uploadedImage || uploadedImage == "") {
          console.log("UPLOADED IMAGE EMPTY!");
        }
        setStoreError("");
        setFileList([]);
        setIsModalOpen(false);
        ...
        updateStoreErrorMesssage == "Store not found !"
      ) {
        ...
        updateStoreStatus === false &&
        updateStoreErrorMesssage == "Unauthorized request !"
      ) {
        ...
        updateStoreStatus === false &&
        updateStoreErrorMesssage == "duplicate name !"
      ) {
        ...
        updateStoreStatus === false &&
        updateStoreErrorMesssage == "Internal Server Error"
      ) {
        ...
        updateStoreStatus === false &&
        updateStoreErrorMesssage == "Unauthorized"
      ) {
  ...
  const storeDeleteClick=()=>{
    setStoreDeleteError("")
    if(deleteData && deleteData!=""){
       dispatch(deleteStore(deleteData?.id))
    }
  };

  React.useEffect(()=>{

    if(deleteStoreLoading===false){
       if(deleteStoreStatus===true){
            dispatch(resetStore())
            setStoreDeleteError("");
            dispatch(getAllStores());
            setDeleteOpen(false);
            setDeleteData("");
       }else if(deleteStoreStatus===false && deleteStoreErrorMessage==="Unauthorized"){
        dispatch(resetStore())
        setStoreDeleteError("*Your session is expired! please login again.")
       }else if(deleteStoreStatus===false && deleteStoreErrorMessage==="Internal Server Error"){
        dispatch(resetStore())
        setStoreDeleteError("*Something went wrong. Please try again !")
       }
    }
  ...
  return (
    <>
      <div className="edit-store">
        {getStoreData &&
          getStoreData?.length >= 1 &&
          Array.isArray(getStoreData) && (
            <div
              style={{
                width: "100%",
                display: "flex",
            ...
            >
              ...
              >
                You have {getStoreData?.length} stores
              </p>
            </div>
          )}

        <div className="edit-store-container">{displayStores}</div>
      </div>
      ...
      <Modal
        title={
          ...
          >
            <WarningRoundedIcon
              sx={{ color: "#f2a407", marginRight: "0.5rem", fontSize: "30px" }}
            />
            <span
              style={{
                textAlign: "center",
      ...
      >
        ...
        >
          ...
          {storeDeleteError && storeDeleteError!="" &&  <p style={{color:"#fc0f03",fontWeight:"bold",
          fontFamily: "'Roboto', sans-serif",marginBottom:"0.2rem"}}>{storeDeleteError}
          </p>}
          ...
          >
            <button
              className="delete-btn-cancel-store"
              onClick={handleDeleteCancel}
              style={{ marginRight: "0.5rem" }}
            >
              Cancel
            </button>
            <button className="delete-btn-store" onClick={storeDeleteClick}>Delete</button>
          </div>
        </div>
      </Modal>
